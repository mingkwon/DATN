...
// Thay toàn bộ đoạn submit modal bằng đoạn này (đã fix getInstance + thêm jQuery fallback)
document.getElementById('bookingForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            // TẮT MODAL – FIX LỖI getInstance
            const modalElement = document.getElementById('bookModal');
            if (typeof bootstrap !== 'undefined' && bootstrap.Modal && bootstrap.Modal.getInstance) {
                bootstrap.Modal.getInstance(modalElement)?.hide();
            } else {
                $(modalElement).modal('hide'); // jQuery fallback
            }

            // Hiện thông báo thành công
            const alert = `
                <div class="alert alert-success text-center alert-dismissible fade show mb-4">
                    <strong>Thành công! ${data.message}</strong>
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>`;
            document.querySelector('#book-table > .container').insertAdjacentHTML('afterbegin', alert);

            // Tải lại sơ đồ → bàn đỏ ngay lập tức
    ...
    .catch(err => {
        console.log(err);
        alert(err.message || 'Vui lòng điền đầy đủ thông tin!');
    });
});
</script>